...
export class GameEngine {
  ...
  constructor(canvas: HTMLCanvasElement, callbacks: GameEngineCallbacks) {
    ...
    this.scene.background = new THREE.Color(0x87ceeb); // Azure desert sky
    this.scene.fog = new THREE.FogExp2(0xdfe9f3, 0.007);

    // Camera (6DOF Flight Camera)
    this.camera = new THREE.PerspectiveCamera(75, canvas.clientWidth / canvas.clientHeight, 0.1, 800);
  ...
  private initLightingAndSky() {
    const ambient = new THREE.AmbientLight(0xfff4e0, 0.95);
  ...
  private initCarpetCockpit() {
    ...
    // Ornate floral diamond medallion
    ...
    // Fringe tassels on front edge
    ctx.fillStyle = '#fff8dc';
    for (let x = 16; x < 240; x += 12) {
      ctx.fillRect(x, 0, 4, 12);
      ctx.fillRect(x, 244, 4, 12);
    }
  ...
  // ==========================================
  // WORLD ENTITY SPAWNER
  // ==========================================

  private spawnInitialWorldEntities() {
    // 1. Initial Mana Spheres scattered across the dunes and sea
    for (let i = 0; i < 24; i++) {
      const x = (Math.random() - 0.5) * 180;
      const z = (Math.random() - 0.5) * 180;
      const groundY = this.terrain.getHeightAt(x, z);
      this.spawnManaSphere(x, Math.max(1, groundY + 1.5), z, 10, 'NEUTRAL');
    }

    // 2. Sea Wyrms in deep ocean bays
    this.spawnCreature('WYRM', 'Sea Wyrm Leviathan', -60, 0, -50, 'NEUTRAL', 80, 40);
    this.spawnCreature('WYRM', 'Abyssal Serpent', 75, 0, -60, 'NEUTRAL', 80, 40);

    // 3. Griffins flying in the mountain skies
    this.spawnCreature('GRIFFIN', 'Sky Roc Griffin', -40, 25, 60, 'NEUTRAL', 60, 30);
    this.spawnCreature('GRIFFIN', 'Golden Griffin', 50, 30, 40, 'NEUTRAL', 60, 30);

    // 4. Skeletons on desert dunes
    this.spawnCreature('SKELETON', 'Undead Raider', -20, 8, -10, 'NEUTRAL', 35, 15);
    this.spawnCreature('SKELETON', 'Bone Warrior', 30, 6, 20, 'NEUTRAL', 35, 15);

    // 5. Rival Wizard: Vhole the Warlock (with rival castle)
    this.spawnCreature('RIVAL_WIZARD', 'Vhole the Warlock', 60, 18, -45, 'RIVAL', 150, 100);
    this.buildCastle(60, -45, 'RIVAL');
  }
  ...
  private updatePlayerFlight(dt: number) {
    ...
    const accel = isBoosted ? 35.0 : 20.0;
    const maxSpd = isBoosted ? 45.0 : this.player.maxSpeed;
    const turnRate = 2.4;

    // Pitch & Yaw turning
    if (this.keysDown.has('KeyA') || this.keysDown.has('ArrowLeft')) {
      this.player.yaw += turnRate * dt;
      this.player.roll = THREE.MathUtils.lerp(this.player.roll, 0.45, dt * 5.0); // Bank left
    } else if (this.keysDown.has('KeyD') || this.keysDown.has('ArrowRight')) {
      this.player.yaw -= turnRate * dt;
      this.player.roll = THREE.MathUtils.lerp(this.player.roll, -0.45, dt * 5.0); // Bank right
    } else {
      this.player.roll = THREE.MathUtils.lerp(this.player.roll, 0, dt * 6.0); // Return level
    }
    ...
    // Bound to world
    const half = this.terrain.size / 2 - 5;
  ...
  private updateManaSpheres(dt: number) {
    for (let i = this.manaSpheres.length - 1; i >= 0; i--) {
      ...
      if (s.y <= groundY + 0.6) {
        s.y = groundY + 0.6;
        s.vy = Math.abs(s.vy) * 0.6; // Bouncing elasticity
        s.vx *= 0.85;
        s.vz *= 0.85;
      }
  ...
  public spawnBalloon(castle: Castle) {
    ...
    // 3D Hot Air Balloon Mesh
    const group = new THREE.Group();

    // Striped Envelope
    const envelopeGeo = new THREE.SphereGeometry(2.2, 16, 16);
    envelopeGeo.scale(1, 1.35, 1);
    const col = castle.owner === 'PLAYER' ? 0x00bcd4 : 0xe53935;
    const envelopeMat = new THREE.MeshLambertMaterial({ color: col });
    const envelope = new THREE.Mesh(envelopeGeo, envelopeMat);
    envelope.position.y = 2.4;
    group.add(envelope);

    // Basket
    const basketGeo = new THREE.CylinderGeometry(0.6, 0.5, 0.8, 8);
  ...
  private updateBalloons(dt: number) {
    this.balloons.forEach(b => {
      ...
      // 1. If cargo is full, return to castle
      ...
      if (b.state === 'RETURNING') {
        ...
        if (dist < 4.0) {
          // Deposit cargo into Castle
          castle.storedMana += b.cargo;
          this.callbacks.onManaDeposit(b.cargo, b.owner);
          if (b.owner === 'PLAYER') {
            soundManager.playManaClaim();
            this.callbacks.onLogMessage(`Your Balloon deposited +${b.cargo} Mana into Castle Treasury!`, 'mana');
      ...
      // 2. Find closest owned mana sphere
  ...
  public buildCastle(x: number, z: number, owner: Faction) {
    ...
    // 3D Castle Keep Mesh
    const group = new THREE.Group();
    const keepGeo = new THREE.CylinderGeometry(3.0, 3.8, 6.0, 8);
    const col = owner === 'PLAYER' ? 0x1e88e5 : 0xd32f2f;
    const keepMat = new THREE.MeshLambertMaterial({ color: col });
    const keep = new THREE.Mesh(keepGeo, keepMat);
    keep.position.y = 3.0;
    group.add(keep);

    // Spire
    const spireGeo = new THREE.ConeGeometry(2.4, 4.0, 8);
    ...
    if (owner === 'PLAYER') {
      ...
      this.callbacks.onLogMessage('Your Castle has been established! A Mana Balloon has launched to harvest possessed spheres.', 'castle');
    }
  }
  ...
  private updateCastles(dt: number) {
    ...
    this.castles.forEach(c => {
      // Check Castle Auto-Upgrade based on stored mana
      if (c.level < 5 && c.storedMana >= c.level * 100) {
        ...
        if (c.owner === 'PLAYER') {
          ...
          soundManager.playCastleLevelUp();
          this.callbacks.onLogMessage(`Castle upgraded to Level ${c.level}! Citadel defenses & mana capacity expanded!`, 'castle');
        }

        // Scale up visual mesh
        const group = this.castleMeshes.get(c.id);
        if (group) {
          group.scale.set(1 + c.level * 0.25, 1 + c.level * 0.25, 1 + c.level * 0.25);
        }
      }
      ...
      if (now - c.lastTurretFire > c.turretCooldown) {
        ...
        // Target hostile creatures or rival players within 40 units
    ...
    // Check Realm Restoration Victory Condition
    const playerCastle = this.castles.find(c => c.owner === 'PLAYER');
    if (playerCastle) {
  ...
  public spawnCreature(
    ...
    if (type === 'WYRM') {
      // Aquatic Serpent Body
      const wyrmGeo = new THREE.CylinderGeometry(0.8, 1.2, 8.0, 8);
      const wyrmMat = new THREE.MeshLambertMaterial({ color: 0x00897b });
      const body = new THREE.Mesh(wyrmGeo, wyrmMat);
      ...
      eye.position.set(0, 0.8, 3.8);
      group.add(eye);
    } else if (type === 'GRIFFIN') {
      // Winged Raptor
      const bodyGeo = new THREE.ConeGeometry(1.2, 4.0, 6);
      const bodyMat = new THREE.MeshLambertMaterial({ color: 0x8d6e63 });
      const body = new THREE.Mesh(bodyGeo, bodyMat);
      body.rotation.x = Math.PI / 2;
      group.add(body);

      // Wings
      const wingGeo = new THREE.BoxGeometry(6.0, 0.2, 1.8);
      const wingMat = new THREE.MeshLambertMaterial({ color: 0xd7ccc8 });
      const wings = new THREE.Mesh(wingGeo, wingMat);
      wings.position.set(0, 0.3, 0);
      group.add(wings);
    } else if (type === 'RIVAL_WIZARD') {
      // Rival Flying Carpet
      const rugGeo = new THREE.PlaneGeometry(2.2, 3.0);
      rugGeo.rotateX(-Math.PI / 2);
      const rugMat = new THREE.MeshLambertMaterial({ color: 0xb71c1c });
      const rug = new THREE.Mesh(rugGeo, rugMat);
      group.add(rug);

      // Wizard Silhouette
      const wizGeo = new THREE.CylinderGeometry(0.4, 0.7, 2.2, 8);
      const wizMat = new THREE.MeshLambertMaterial({ color: 0x212121 });
      const wiz = new THREE.Mesh(wizGeo, wizMat);
      wiz.position.set(0, 1.1, 0);
      group.add(wiz);
    } else {
      // Skeleton / Troll
      const skelGeo = new THREE.BoxGeometry(1.2, 2.6, 1.2);
      const skelMat = new THREE.MeshLambertMaterial({ color: 0xeeeeee });
  ...
  private updateCreatures(dt: number) {
    ...
    for (let i = this.creatures.length - 1; i >= 0; i--) {
      ...
      if (c.health <= 0) {
        ...
        // Erupt into Mana Spheres!
        this.spawnManaSphere(c.x, c.y + 1.5, c.z, c.manaReward, 'NEUTRAL');
        if (group) this.scene.remove(group);
        this.creatureMeshes.delete(c.id);
        this.creatures.splice(i, 1);
        this.callbacks.onLogMessage(`${c.name} was vanquished! Erupted into +${c.manaReward} Mana Spheres!`, 'combat');
        continue;
      }
      ...
      if (c.type === 'WYRM') {
        // Sea Wyrm leaps from water and shoots venom
        c.animTimer += dt;
        c.y = Math.sin(c.animTimer * 1.5) * 4.0; // Leaping motion

        if (distToPlayer < 40 && now - c.lastAttackTime > c.attackCooldown) {
        ...
        // Flight circle and dive attack
        c.yaw += 0.8 * dt;
        c.x += Math.sin(c.yaw) * 16 * dt;
        c.z += Math.cos(c.yaw) * 16 * dt;

        if (distToPlayer < 30 && now - c.lastAttackTime > c.attackCooldown) {
          c.lastAttackTime = now;
          ...
          if (this.player.activeShieldTimer <= 0) {
            ...
            this.callbacks.onLogMessage('A Golden Griffin swoops down and claws your carpet! (-14 HP)', 'combat');
          }
        }
      } else if (c.type === 'RIVAL_WIZARD') {
        // Rival Wizard flies around, shoots fireballs, and claims mana!
        const dx = this.player.position.x - c.x;
        const dz = this.player.position.z - c.z;
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (p.y <= groundH || p.y <= 0) {
        ...
        if (p.type === 'METEOR') {
          ...
          this.callbacks.onLogMessage('METEOR IMPACT! A massive crater is blasted into the earth!', 'cataclysm');
  ...
  public spawnVolcano(x: number, z: number) {
    ...
    this.callbacks.onLogMessage('RAISE VOLCANO: A towering volcanic mountain erupts from the depths!', 'cataclysm');
  }
  ...
  public castSpell(spellId: SpellId) {
    ...
    switch (spellId) {
      ...
      case 'LIGHTNING':
        soundManager.playLightning();
        // Instant bolt to closest creature in crosshair
        ...
        if (target) {
          ...
          this.callbacks.onLogMessage('LIGHTNING arcs through the storm clouds!', 'magic' as any);
        }
        break;

      case 'POSSESS':
        soundManager.playPossess();
        // Claim mana spheres in front of player
        let claimedCount = 0;
      ...
      case 'HEAL':
        ...
        this.callbacks.onLogMessage('Pure mana restores +40 Health to your life force.', 'info');
        break;

      case 'SPEED':
        soundManager.playFireballCast();
        this.player.activeSpeedTimer = 10.0;
        this.callbacks.onLogMessage('AFTERBURNER SPEED: Flight thrust doubled for 10s!', 'info');
        break;

      case 'SHIELD':
        soundManager.playPossess();
        this.player.activeShieldTimer = 8.0;
        this.callbacks.onLogMessage('ARCANE SHIELD: Invulnerability bubble activated for 8s!', 'info');
        break;
      ...
      case 'CRATER':
        ...
        this.callbacks.onLogMessage('CRATER: Ground shattered and displaced!', 'cataclysm');
        break;
      ...
      case 'EARTHQUAKE':
        ...
        soundManager.playVolcanoErupt();
        this.screenShake = 1.0;
        this.callbacks.onLogMessage('EARTHQUAKE: The realm splits along tectonic fault lines!', 'cataclysm');
        break;
